...
```{r import-data}
app_data <- read_parquet("C:/Users/mehri/Downloads/app_data_sample.parquet")
edge_data <- vroom("C:/Users/mehri/Downloads/edges_sample.csv", delim = ",")
```
...
app_data <- app_data %>%
  ...
  mutate(tenure = max(filing_date) - min(filing_date)) %>%
  ungroup()
```
...
```{r prepare-edge-data}
edge_data <- edge_data %>%
  select(ego_examiner_id, alter_examiner_id, advice_date)
```
...
```{r preparing-data-for-network}
## Identify and Add Missing Examiners
...
missing_examiners <- tibble(
  examiner_id = missing_ids,
  gender = NA_character_, # Adjust based on actual data type in 'app_data'
  race = NA_character_, # Adjust based on actual data type in 'app_data'
  tenure = NA_real_ # Adjust based on actual data type in 'app_data'
  # Add more columns here as placeholders to match the structure of 'app_data'
)

# Ensure 'missing_examiners' has all the columns present in 'app_data', filled with NA
additional_columns <- setdiff(names(app_data), names(missing_examiners))
...
```{r tenure-dist}
tenure_dist <- app_data %>%
  summarise(mean_tenure = mean(tenure), sd_tenure = sd(tenure))
```
...
```{r network-visualization, fig.cap="USPTO Advice Network by Gender"}
set.seed(123)
ggraph(g, layout = "fr") +
  geom_edge_link() +
  ...
  theme_graph() +
  labs(title = "USPTO Advice Network", color = "Gender")
```
